...
## Grounding contract (v1.4)

The continuous `transcription.original_script` is mandatory. Transcribe the complete supplied
act region once, in original reading order, before structuring any observation. Do not replace
the continuous transcription with a summary, translation, field list, or selected excerpts.

Every observation whose `observation_state` is `PRESENT` must carry a non-empty
`original_script` copied verbatim as one continuous substring of that transcription. It may
include uncertainty notation only if the identical notation appears in the transcription. Do
not use an English normalization, ellipsis, paraphrase, or reconstructed wording as
`original_script`. For a number or symbol on a Russian-language act, include enough adjacent
inked wording that the supporting excerpt contains Cyrillic. If no supporting excerpt can be
copied, do not assert PRESENT; use the honest typed state that describes the evidence.

Before returning JSON, mechanically self-check every PRESENT observation: normalize Unicode NFC
and collapse whitespace only, then confirm its complete `original_script` occurs in the
continuous transcription. A failed check means the field must be corrected or withdrawn.
